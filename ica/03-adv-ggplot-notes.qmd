...
- Relate the intro paragraph: "Scientists declared that 2015 was Earth’s hottest year on record..." to the *design* of the visualization.  In particular, based on the intro paragraph,
  ...
  NYTimes wants to convey that climate change is a serious issue, with global temperatures on the rise every year. 
  They convey this information by showing that temperatures across US cities are higher than the "normal" range, with many cities reaching record high temperatures.

**Aesthetic Mapping** 

- What specific variables (from the data codebook) underlie the visualization?

Month, Day, DateInYear, RecordPrecip, NormalHigh/NormalLow, RecordLow`/`RecordHigh

- How do these variables map to aesthetics of the visual elements, eg, position, size, shape, and color of glyphs?

DateInYear is the x axis, ymin and max are NormalHigh/NormalLow, RecordLow`/`RecordHigh
...
ggplot(weather, aes(x= dateInYear, ymin= RecordLow, ymax= RecordHigh))+
  geom_linerange(color = "#ECEBE3")  +
  geom_linerange(
    aes(dateInYear, ymin= NormalLow, ymax= NormalHigh), color= "#C8B8BA")+
  geom_linerange(
    aes(dateInYear, ymin= Low, ymax= High), color= "#A90248")+
    theme_classic() 
...
ggplot(weather, aes(x= dateInYear, ymin= RecordLow, ymax= RecordHigh))+
  geom_vline(data = month_lines, aes(xintercept = dateInYear), color = "grey", linewidth = 0.2, linetype = "dotted")+
  geom_linerange(color = "#ECEBE3")  +
  geom_linerange(
    aes(dateInYear, ymin= NormalLow, ymax= NormalHigh), color= "#C8B8BA")+
  geom_linerange(
...
It is important to be critical of AI generated code. I try to read it line by line and make sure it is correct and is mapping the right thing. 

```{r}
month_labels <- weather |> 
  mutate(month = month(date, label = TRUE, abbr = FALSE)) |> 
  group_by(month) |> 
  summarise(midpoint = mean(dateInYear))

month_lines <- weather |> 
  filter(day(date) == 1)

plot_1 <- ggplot(weather, aes(x= dateInYear, ymin= RecordLow, ymax= RecordHigh))+
  geom_vline(data = month_lines, aes(xintercept = dateInYear), color = "grey", linewidth = 0.2, linetype = "dotted")+
  geom_linerange(color = "#ECEBE3")  +
  geom_linerange(
    aes(dateInYear, ymin= NormalLow, ymax= NormalHigh), color= "#C8B8BA")+
...
plot_2 <- ggplot(weather_2, aes(x = dateInYear, y = CulmPrec)) +
  geom_area(aes(group = month), fill = "#ebeae2") +
  geom_line(aes(group = month), color = "#32a3d8", linewidth = 0.6) +
  geom_point(data = records, aes(x = dateInYear, y = CulmPrec),
             shape = 25, fill = "black", color = "black", size = 2.5) + geom_text(data = month_tot, aes(x = x, y = y, label = y),
            vjust = -0.4, size = 3.5) +
  theme_classic()
...
plot_1 / plot_2 + plot_layout(heights = c(2, 1))
```
