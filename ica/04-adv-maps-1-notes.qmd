...
mn_cities <- st_read("../ica/data/04_adv_maps/shp_loc_pop_centers/")

mn_water <- st_read("../ica/data/04_adv_maps/shp_water_lakes_rivers/")
```
...
transform_and_plot(nc, new_crs = "ESRI:102003", new_title= "North Carolina Map Projection 2")

transform_and_plot(nc, new_crs = "EPSG:32100", new_title= "North Carolina Map Projection 3")
...
ggplot(mn_counties_transformed) +
    geom_sf(data = mn_counties_transformed, fill = "white") +
    geom_sf(data = mn_cities, size = 0.2) +
    theme_classic() + ggthemes::theme_map()
```
...
It provides colour maps that are perceptually uniform in both colour and black-and-white. It is useful to make maps accessible to viewers with common forms of colour blindness.

- Look through the examples section. What is the difference between the `_d()`, `_c()`, and `_b()` variants of this function?

viridis_d with discrete data
viridis_b to bin continuous data before mapping
viridis_c with continuous data
